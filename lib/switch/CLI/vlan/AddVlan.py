##########################################################################################
# Name:        switch.CLI.interface.AddVlan.py
#
...
def AddVlan(**kwargs):
    ...
    if retCode != 0:
        if config:
            lib.LogOutput('error', "Failed to create VLAN or failed to get into VLAN context."+ command)
        else:
            lib.LogOutput('error', "Failed to delete VLAN."+ command)
        returnCls = lib.returnStruct(returnCode=1, buffer=bufferString)
        return returnCls
    else:
        if config:
            lib.LogOutput('debug', "Created VLAN or entered VLAN context." + command)
        else:
            lib.LogOutput('debug', "Deleted VLAN." + command)
