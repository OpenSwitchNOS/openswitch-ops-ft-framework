...
def InterfaceStatisticsShow(**kwargs):
    ...
    lib.LogOutput('info', "Show interface statistics.*****"+ command)    
    ...
    if retCode != 0:
        lib.LogOutput('error', "Failed to get information ."+ command)
    ...
    for curLine in overallBuffer:
        bufferString += str(curLine)
    returnCls = lib.returnStruct(returnCode=returnCode, buffer=bufferString, data=data)
    return returnCls    
