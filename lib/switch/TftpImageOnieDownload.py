...
class TftpImageOnieDownload() :
  ...
  def __init__(self,device):
      ...
      LogOutput("info","Dynamically add the Tftp uplink port to  device(DUT)")
      returnUplink = RTL.UplinkPortAdd(device=device)
      returnCode = returnUplink.returnCode() 
      TftpUplinkIP = returnUplink.valueGet(key="IP")
      if TftpUplinkIP is None :
       LogOutput('error', "Failed to get the TFTP IP for the uplink")
       self.AddUplink = 1
      else :
       self.TftpUplinkIP = TftpUplinkIP.replace("\"","")
      if returnCode != 0:
        LogOutput('error', "Failed to add TFTP uplink to device")
        self.AddUplink = 1
  ...
  def  TftpOperation(self,**kwargs):
     ...
     if connection:
       LogOutput("info","Switch rebooted to Onie prompt for image upgrade")
     else :
       LogOutput("error","Switch not rebooted to Onie prompt for image upgrade")
     ...
     if ExitFlag1 == 0 and ExitFlag2 == 0 and ExitFlag3 == 0 and ExitFlag4 ==0 :
      ...
      LogOutput("info","Change the permissions of the image and run the image executable file***")
      devIntRetStruct = switch.DeviceInteract(connection=connection, command=command)
      self.returnCode = devIntRetStruct.get('returnCode')
      self.returnDict['ImageState'] = devIntRetStruct.get('buffer')
      if self.returnCode != 0:
       LogOutput('error', "Failed to change permissions of the image and run the image executable file")
       ExitFlag3 = 1
     ...
     if ExitFlag1 == 0 and ExitFlag2 == 0 and ExitFlag3 == 0 and ExitFlag4 ==0 :
      ...
      if self.returnCode != 0:
       LogOutput('error', "Failed to  run the image executable file")
       ExitFlag4 = 1
     #Reboot the switch again to install the image 
     LogOutput("info","Reboot the switch again to install the image::")
     connection = switch.Reboot(connection=connection)
     if connection:
       LogOutput("info","Switch rebooted for image upgrade")
     else :
       LogOutput("error","Switch not rebooted for image upgrade")
       self.returnDict = "None"
       self.returnCode = 1
       ExitFlag1 = 1   
 
     #Remove the TFTP uplink after rebooting the device to the new image
     retRmUplink = self.RemoveUplink(device=self.device) 
     returnCode = retRmUplink.returnCode
     if returnCode != 0:
        LogOutput('error', "Failed to remove TFTP uplink to device")
        return retRmUplink
 
     #Return results
     returnCls = returnStruct(returnCode=self.returnCode, data=self.returnDict)
     return returnCls 
     
  def RemoveUplink(self,device): 
     LogOutput('info',"Removing the TFTP uplink")
