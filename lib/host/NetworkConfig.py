...
#              broadcast  - broadcast to be configured
...
def NetworkConfig(**kwargs):
   ...
   # Send the command
    ...
    if clear:
        ...
        command = ETH_INTERFACE_CFGIP_CMD % (eth, ipAddr, netMask,
                broadcast)
        returnStruct = host.DeviceInteract(connection=connection,
                command=command)
        retCode = returnStruct.get('returnCode')
