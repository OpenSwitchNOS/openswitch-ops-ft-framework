   ...
   if ipv6Flag:
        try:
                ...
                if ipAddr.find("fe80") == -1:
                        command = "ping6 %s -c %d -s %d"%(ipAddr,packetCount,packetSize)
                else:
                        command = "ping6 -I %s %s -c %d -s %d"%(eth,ipAddr,packetCount,packetSize)
        except  socket.error:
                returnCode = FAILED
   else:
        try:
                socket.inet_pton(socket.AF_INET,ipAddr)
                command = "ping %s -c %d -s %d"%(ipAddr,packetCount,packetSize)
        except  socket.error:
                returnCode = FAILED
