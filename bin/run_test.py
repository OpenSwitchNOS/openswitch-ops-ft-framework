...
entfwbase = os.environ['FT_FRAMEWORK_ENT']
if entfwbase != "":
    appendDir = entfwbase + "/lib"
    sys.path.append(appendDir)
    if pythonPathString != "":
        pythonPathString += ":"+appendDir
    else:
...
if retCode['returnCode'] == 0 :
    ...
    try :
        ...
        exit(1)
#     if args.phystopo != "virtual":
#        if isinstance(args.phystopo, int) is False:
#           try :
#              shutil.copy(TopologyXMLPath,headers.ResultsDirectory['resultsDir'])
#           except :
#              common.LogOutput('error',"Topology file not copied to destination ->"+headers.ResultsDirectory['resultsDir'])
#              exit(1)
else :
