...
# 
    ...
    if retStruct.returnCode() != 0:
        LogOutput('error', "Failed to ping from workstation 1 to workstation2 through the switch")
        retCode = 1
        assert(False)
    else:
...
# 
    ...
    if retStruct.returnCode() != 0:
        LogOutput('error', "Failed to unconfigure IP address on dut01 port " + str(dut01.linkPortMapping['lnk01']))
        retCode = 1
        assert(False)
    else:
    ...
    if retStruct.returnCode() != 0:
        LogOutput('error', "Failed to unconfigure IP address on dut01 port " + str(dut01.linkPortMapping['lnk02']))
        retCode = 1
        assert(False)
    else:
    ...
    retStruct = InterfaceEnable(deviceObj=dut01, enable=False,
                             interface=dut01.linkPortMapping['lnk01'])
    if retStruct.returnCode() != 0:
        LogOutput('error', "Failed to shutdown dut01 interface " + str(dut01.linkPortMapping['lnk01']))
        retCode = 1
        assert(False)
    else:
    ...
    retStruct = InterfaceEnable(deviceObj=dut01, enable=False,
                                interface=dut01.linkPortMapping['lnk02'])
    if retStruct.returnCode() != 0:
        LogOutput('error', "Failed to shutdown dut01 interface " + str(dut01.linkPortMapping['lnk02']))
        retCode = 1
        assert(False)
    else:
