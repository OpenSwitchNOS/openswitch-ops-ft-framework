###PROC+#####################################################################
# Name:        opstestfw.switch.PacketCapture
#
# Namespace:   switch
#
...
class PacketCapture() :
  ...
  def StartCapture(self,**kwargs):
    ...
    returnStruct = opstestfw.host.DeviceInteract(connection=connection, command=command)
  ...
  def ParseCapture(self,connection,**kwargs) :
    ...
    returnCode = opstestfw.host.SCPFileTransfer(self.device,self.filename,filepath,localpath)
    if returnCode != 0 :
