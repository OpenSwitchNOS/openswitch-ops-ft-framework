...
class Device ():
    ...
    def Connect(self):
        ...
        if rsvnType == 'virtual':
            ...
            ExpectInstance = opstestfw.switch.ExpectLog.DeviceLogger(expectFileString)
            ...
            if rsvnType == 'virtual':
                self.expectHndl = pexpect.spawn(telnetString, echo=False, logfile=opstestfw.switch.ExpectLog.DeviceLogger(expectLogFile))
    ...
    def DeviceInteract(self, **kwargs):
        ...
        if errorCheck is True and returnCode == 0 and ErrorFlag == None :
            #errorCheckRetStruct = opstestfw.switch.ErrorCheck(connection=connection, buffer=santString)
        ...
        if ErrorFlag == 'CLI' :
            #errorCheckRetStruct = opstestfw.switch.CLI.ErrorCheck(connection=connection, buffer=santString)
            #returnCode = errorCheckRetStruct['returnCode']
            LogOutput('debug', "NEED TO FIX")
            #The following file checks for errors in Onie prompts after analyzing Onie expect buffer
        if ErrorFlag == 'Onie' :
            #errorCheckRetStruct = opstestfw.switch.ErrorCheckOnie(connection=connection, buffer=santString)
            #returnCode = errorCheckRetStruct['returnCode']
            LogOutput('debug', "NEED TO FIX")
    ...
    def ErrorCheck(self, **kwargs):
        ...
        if index == 0 or index == 1:
            ...
            LogOutput('error', "Received timeout in opstestfw.switch.ErrorCheck")
            retStruct['returnCode'] = 1
            return retStruct
