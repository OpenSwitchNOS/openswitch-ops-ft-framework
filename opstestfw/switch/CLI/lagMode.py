...
def lagMode(**kwargs):
    ...
    #Note this command is not the core of the function, but it is so important that if the initial status of the LAG mode cannot be determined, the other will always fail
    command = 'show lacp aggregates lag' + str(lagId)
    returnDevInt = deviceObj.DeviceInteract(command=command)
    finalReturnCode = returnDevInt['returnCode']
    overallBuffer.append(returnDevInt['buffer'])
    if finalReturnCode!= 0:
        opstestfw.LogOutput('error', "Could not obtain LAG LACP mode initial status")
        bufferString = ""
        for curLine in overallBuffer:
            bufferString += str(curLine)
        returnCls = opstestfw.returnStruct(returnCode=returnCode, buffer=bufferString)
        ...
        if result is None:
            opstestfw.LogOutput('error', "Could not identify LAG LACP mode initial status")
            returnCls = opstestfw.returnStruct(returnCode=1, buffer=bufferString)
            return returnCls
        else:
            initialStatus = result.group(1)
            opstestfw.LogOutput('debug', "LAG LACP mode initial status identified as " + initialStatus)
            
        #Verify if the LAG was already in static mode and will be changed again
        if initialStatus == lacpMode and lacpMode == 'off':
            opstestfw.LogOutput('debug', "LACP mode is set to off already. No change is performed")
        else:
            ...
            overallBuffer.append(returnDevInt['buffer'])
            if returnCode != 0:
                opstestfw.LogOutput('error', "Failed to enter LAG" + str(lagId) + " configuration context on device " + deviceObj.device)
                bufferString = ""
                for curLine in overallBuffer:
                    bufferString += str(curLine)
                returnCls = opstestfw.returnStruct(returnCode=returnCode, buffer=bufferString)
            else:
                opstestfw.LogOutput('debug', "Entered LAG" + str(lagId) + " configuration context on device " + deviceObj.device)
            ...
            overallBuffer.append(returnDevInt['buffer'])
            if finalReturnCode != 0:
                opstestfw.LogOutput('error', "Failed to configure LACP mode to " + lacpMode + " on device " + deviceObj.device)
            else:
                opstestfw.LogOutput('debug', "Changed LACP mode to " + lacpMode + " on device " + deviceObj.device)
