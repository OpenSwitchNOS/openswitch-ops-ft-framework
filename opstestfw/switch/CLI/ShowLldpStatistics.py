...
def ShowLldpStatistics(**kwargs):
    ...
    if returnCode != 0:
        ...
        for line in bufferSplit:
            TotalPacketsTransmitted = re.match("^Total\sPackets\stransmitted\s*:\s*(\d+)\s*$",line)
            if TotalPacketsTransmitted:
                globalStatsDict['Total_Packets_Transmitted'] = TotalPacketsTransmitted.group(1)
                continue
            TotalPacketsReceived = re.match("^Total\sPackets\sreceived\s*:\s*(\d+)\s*$",line)
            if TotalPacketsReceived:
                globalStatsDict['Total_Packets_Received'] = TotalPacketsReceived.group(1)
                continue
            TotalPacketsReceivedAndDiscarded = re.match("^Total\sPacket\sreceived\sand\sdiscarded\s*:\s*(\d+)\s*$",line)
            if TotalPacketsReceivedAndDiscarded:
                globalStatsDict['Total_Packets_Received_And_Discarded'] = TotalPacketsReceivedAndDiscarded.group(1)
                continue
            TotalTLVsUnrecognized = re.match("^Total\sTLVs\sunrecognized\s*:\s*(\d+)\s*$",line)
            if TotalTLVsUnrecognized:
                globalStatsDict['Total_TLVs_Unrecognized'] = TotalTLVsUnrecognized.group(1)
            #Need to add LLDP Port Statistics

            returnDict['globalStats'] = globalStatsDict
    #LogOutput('info', "####"+globalStatsDict)
