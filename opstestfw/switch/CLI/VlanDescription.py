##########################################################################################
# Name:        opstestfw.switch.CLI.interface.VlanDescription.py
#
...
def VlanDescription(**kwargs):
    ...
    if deviceObj is None or description is None or vlanId is None:
        opstestfw.LogOutput('error', "Need to pass switch device object deviceObj, VLAN id vlanId and VLAN description description to this routine")
    ...
    if retCode != 0:
        opstestfw.LogOutput('error', "Failed to get into vlan prompt."+ command)
        returnCls = opstestfw.returnStruct(returnCode=1, buffer=bufferString)
        return returnCls
    else:
    ...
    if retCode != 0:
        if config:
            opstestfw.LogOutput('error', "Failed to set the VLAN description."+ command)
        else:
            opstestfw.LogOutput('error', "Failed to remove the VLAN description."+ command)
        returnCls = opstestfw.returnStruct(returnCode=1, buffer=bufferString)
        return returnCls
    else:
