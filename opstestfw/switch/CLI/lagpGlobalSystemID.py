##########################################################################################
# Name:        	opstestfw.switch.CLI.lag.lagpGlobalSystemID
#
...
def lagpGlobalSystemID(**kwargs):
    ...
    if deviceObj is None or systemID is None:
        opstestfw.LogOutput('error', "Need to pass switch deviceObj and systemID to this routine")
    ...
    overallBuffer.append(returnStructure['buffer'])
    if retCode != 0:
        opstestfw.LogOutput('error', "Failed to configure LACP system ID: " + str(systemID))
    else:
        opstestfw.LogOutput('debug', "LACP system ID configured: " + str(systemID))
