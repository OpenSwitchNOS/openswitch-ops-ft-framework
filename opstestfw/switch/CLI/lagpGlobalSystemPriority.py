########################################################################################
# Name:        opstestfw.switch.CLI.lag.lagpGlobalSystemPriority
#
...
def lagpGlobalSystemPriority(**kwargs):
        ...
        if deviceObj is None or systemPriority is None:
            opstestfw.LogOutput('error', "Need to pass switch deviceObj and systemPriority to this routine")
        ...
        overallBuffer.append(returnStructure['buffer'])
        if retCode != 0:
            opstestfw.LogOutput('error', "Failed to configure LACP system priority: " + str(systemPriority))
        else:
            opstestfw.LogOutput('debug', "LACP system priority configured: " + str(systemPriority))
        ...
        overallBuffer.append(returnStructure.buffer())
        if returnCode != 0:
            opstestfw.LogOutput('error', "Failed to exit configure terminal prompt")
            bufferString = ""
            for curLine in overallBuffer:
                bufferString += str(curLine)
