##########################################################################################
# Name:        opstestfw.switch.CLI.lag.lagFallback
#
...
def lagFallback(**kwargs):
    ...
    if finalReturnCode != 0:
        if fallbackFlag is True:
            opstestfw.LogOutput('error', "Failed to enable LACP fallback on interface lag " + str(lagId) + " on device " + deviceObj.device)
        else:
            opstestfw.LogOutput('error', "Failed to disable LACP fallback on interface lag " + str(lagId) + " on device " + deviceObj.device)
    else:
        if fallbackFlag is True:
            opstestfw.LogOutput('debug', "Enabled LACP fallback on interface lag " + str(lagId) + " on device " + deviceObj.device)
        else:
            opstestfw.LogOutput('debug', "Disabled LACP fallback on interface lag " + str(lagId) + " on device " + deviceObj.device)
