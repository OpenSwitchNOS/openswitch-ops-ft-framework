...
def LoopbackInterfaceEnable(**kwargs):
    ...
    if addr is not None:
        ...
        overallBuffer.append(returnStructure['buffer'])
        if retCode != 0:
            LogOutput('error', "Failed to assign ip to loopback interface " + str(loopback))
        else:
            LogOutput('debug', "assigned IP to loopback interface " + str(loopback))

    '''
    if addr is not None:
        command = "ipv6 address " + str(addr) + "/" + str(mask)
        returnStructure = deviceObj.DeviceInteract(command=command)
        retCode = returnStructure['returnCode']
        overallBuffer.append(returnStructure['buffer'])
        if retCode != 0:
